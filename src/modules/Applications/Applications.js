...
import { getApplications, deleteApplication } from "../../common/apis/applications";
...
const Applications = () => {
  ...
  const handleEdit = () => {
    setLocalStorage("applications-detail-row", selectedRow);
    navigate(`/scit/newApplication`);
  };
  ...
  const handleDelete = async () => {
    try {
      await deleteApplication(selectedRow.id);
      await refetch();
      setOpenDeleteModal(false);

      toast.success(`You have Successfully deleted the application.`, {
        position: "top-right",
        autoClose: 1000,
      ...
      console.error('Error deleting Application.Please contact our ICT team for further guidance', error);
    }
  };
              ...
              renderTopToolbarCustomActions={({ table }) => (
                ...
                >
                  <Button
                    ...
                    onClick={() => {
                      removeItem('applications-detail-row'); 
                      navigate("/scit/newApplication");
                    }}
                    sx={{
                      fontWeight: "bolder",
